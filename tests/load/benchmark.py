...
async def main() -> None:
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=10, trust_env=False, headers=headers) as client:
    ...
    def percentile(value: float) -> float:
        index = max(0, min(len(ordered) - 1, int(len(ordered) * value) - 1))
