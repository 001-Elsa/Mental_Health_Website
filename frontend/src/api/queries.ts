...
type Visibility = "公开" | "私人" | "私密" | "鍏紑" | "绉佷汉";
type ArticleStatus = "已发布" | "草稿" | "宸插彂甯?" | "鑽夌";

export type RecommendationEmotion = "焦虑" | "低落" | "烦躁" | "平稳" | "愉悦";
...
export const authApi = {
  ...
  notifications: () => api<{ unread_count: number; items: import("../types").Notification[] }>("/api/users/me/notifications"),
...
export const analyticsApi = {
  overview: () =>
    api<{ total_users: number; total_mood_logs: number; total_consultations: number; avg_mood_score: number }>("/api/analytics/overview"),
  moodTrend: (scope: "mine" | "all" = "mine") => api<TrendPoint[]>(`/api/analytics/mood-trend?days=14&scope=${scope}`),
  consultationStats: () => api<TrendPoint[]>("/api/analytics/consultation-stats?days=14"),
  userActivity: () => api<ActivityPoint[]>("/api/analytics/user-activity?days=14"),
  moodForecast: () => api<{
...
export const discussionsApi = {
  ...
  create: (payload: { title: string; category: string; content: string; visibility: Visibility; image_url?: string; audio_url?: string }) =>
  ...
  report: (id: number, reason: string, detail = "") =>
    api<{ ok: boolean; report_id: number }>(`/api/discussions/${id}/reports`, { method: "POST", body: toBody({ reason, detail }) }),
};
...
export const consultApi = {
  ...
  updateConversation: (id: string, payload: { title?: string; pinned?: boolean }) =>
    api<{ conversation_id: string; title: string; pinned: boolean }>(`/api/consult/conversations/${id}`, { method: "PATCH", body: toBody(payload) }),
  setConversationVisibility: (id: string, visibility: Visibility) =>
    api<{ conversation_id: string; visibility: Visibility }>(`/api/consult/conversations/${id}/visibility?visibility=${encodeURIComponent(visibility)}`, { method: "PATCH" }),
  removeConversation: (id: string) => api<{ ok: boolean }>(`/api/consult/conversations/${id}`, { method: "DELETE" }),
  ...
  chat: (payload: { conversation_id: string; message: string; visibility: string; request_key?: string }) =>
    api<ChatResponse>("/api/consult/chat", { method: "POST", body: toBody({ ...payload, request_key: payload.request_key ?? crypto.randomUUID() }) }),
};
...
export const recommendationsApi = {
  ...
  exercises: () => api<{ emotion: RecommendationEmotion; is_manual: boolean; items: import("../types").Exercise[] }>("/api/recommendations/exercises"),
};
...
export const adminApi = {
  ...
  handleReport: (id: number, action: string) => api<{ ok: boolean }>(`/api/admin/reports/${id}`, { method: "PATCH", body: toBody({ action }) }),
  moderation: () => api<Discussion[]>("/api/admin/moderation"),
  moderate: (id: number, action: "approve" | "hide") => api<{ ok: boolean }>(`/api/admin/moderation/${id}?action=${action}`, { method: "PATCH" }),
  users: () => api<AdminUser[]>("/api/admin/users"),
  updateRole: (id: number, role: "student" | "admin") => api<{ ok: boolean }>(`/api/admin/users/${id}/role`, { method: "PATCH", body: toBody({ role }) }),
  words: () => api<SensitiveWord[]>("/api/admin/sensitive-words"),
  addWord: (word: string, category = "unsafe") => api<SensitiveWord>("/api/admin/sensitive-words", { method: "POST", body: toBody({ word, category }) }),
  ...
  updateArticleStatus: (id: number, status: ArticleStatus) => api<{ ok: boolean }>(`/api/admin/articles/${id}/status`, { method: "PATCH", body: toBody({ status }) }),
};

export const knowledgeApi = {
  ask: (question: string) => api<KnowledgeAnswer>("/api/knowledge/ask", { method: "POST", body: toBody({ question }) }),
};
